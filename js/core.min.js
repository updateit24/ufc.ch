...
function loadContent(url, n) {
    $.ajax({
        ...
        success: function(data) {
            // Asegúrate de que estás insertando el contenido en el lugar correcto
            n.page.html(data); // Actualiza el contenido de la página
            n.page.removeClass(n.animOut).addClass(n.animIn); // Añadir animación de entrada

            // Manejar la animación de entrada
            setTimeout(function() {
                n.page.removeClass(n.animIn); // Eliminar clase de animación
                n.onAnimated && n.onAnimated(n); // Callback si existe
            }, n.animDuration);
        },
        error: function() {
            console.error("Error al cargar la página."); // Manejo de errores
...
function pageTransition(n) {
    n = n || {};
    n.page = n.page || $("#page");
    n.animDelay = n.animDelay || 500;
    n.animDuration = n.animDuration || 1000;
    n.animIn = n.animIn || "fadeIn";
    n.animOut = n.animOut || "fadeOut";
    n.onReady = n.onReady || null;
    n.onAnimated = n.onAnimated || null;
    n.conditions = n.conditions || function(n, a) {
        return !/(\#|callto:|tel:|mailto:|:\/\/)/.test(a);
    };

    setTimeout(function() {
        n.onReady && n.onReady(n);
        n.page.addClass(n.animIn + " animated");
        n.page.css("animation-duration", n.animDuration + "ms");
        n.page[0].addEventListener("animationstart", function() {
            setTimeout(function() {
                n.page.removeClass(n.animIn);
                n.onAnimated && n.onAnimated(n);
            }, n.animDuration);
        });
    }, n.animDelay);

    // Manejando clics en los enlaces
    $("a").click(function(a) {
        var t = a.currentTarget.getAttribute("href");
        if (n.conditions(a, t)) {
            a.preventDefault(); // Evitar la acción predeterminada
            console.warn("[page transition]");
            var e = this.href;

            n.page.removeClass(n.animIn).addClass(n.animOut); // Aplica animación de salida

            setTimeout(function() {
                // Cambiar la URL sin recargar la página
                window.history.pushState({ path: e }, '', e);
                
                // Cargar el contenido de la nueva URL usando AJAX
                loadContent(e, n); // Llama a la función para cargar contenido
            }, n.animDuration);
        }
    });
}
...
window.onpopstate = function(event) {
    if (event.state) {
        ...
        loadContent(event.state.path, { page: $("#page") });
    } else {
        // Cargar la página inicial si no hay estado
...
window.onpopstate = function(event) {
    console.log("Evento popstate activado:", event);
    if (event.state) {
        loadContent(event.state.path, { page: $("#page") });
    } else {
        loadContent(window.location.href, { page: $("#page") });
